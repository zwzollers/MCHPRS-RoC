...
impl DeviceConfig {
    ...
    pub fn create_project(&self) -> bool {

        let mut tcl = format!(
        "package require ::quartus::project
project_new -overwrite -revision FPGA-MCHPRS FPGA-MCHPRS 
...
set_global_assignment -name SYSTEMVERILOG_FILE ../src/redstone/RoC.sv
...
set_location_assignment PIN_{i_clk} -to i_clk\n",
        ...
        let mut file = File::create("FPGA/prj/project.tcl").unwrap();
        ...
        let out = Command::new("cmd")
            ...
            .args(&["/C", r"C:\intelFPGA_lite\23.1std\quartus\bin64\quartus_sh -t project.tcl"])
            .output()
            .unwrap();
    ...
    pub fn compile (&self) -> CompilerResults {
        let results = CompilerResults{state: true};
        let out = Command::new("cmd")
            .current_dir("FPGA/prj")
            .args(&["/C", r"C:\intelFPGA_lite\23.1std\quartus\bin64\quartus_sh --flow compile FPGA-MCHPRS"])
            .output()
            .unwrap();
        println!("{:?}", String::from_utf8_lossy(&out.stdout));
    ...
    pub fn program (&self) -> ProgramResults {
        ...
        let out = Command::new("cmd")
            .current_dir("FPGA/prj")
            .arg("/C")
            .raw_arg(r#"C:\intelFPGA_lite\23.1std\quartus\bin64\quartus_pgm -c "DE-SoC [USB-1]" -m jtag -o "p;FPGA-MCHPRS.sof@2""#)
            .output()
            .unwrap();
        println!("{:?}", String::from_utf8_lossy(&out.stdout));
